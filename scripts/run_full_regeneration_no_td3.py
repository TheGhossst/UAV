...
Usage:
  ...
  python scripts/run_full_regeneration_no_td3.py --skip-highstat-n200
  python scripts/run_full_regeneration_no_td3.py --from-step plots
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if at_least("highstat"):
        ...
        for n_runs, area in ((20, 100), (20, 500), (100, 100), (100, 500)):
            cmd = [
                PY,
                "-u",
                "scripts/run_highstat_campaign.py",
                "--n-runs",
                str(n_runs),
                "--area-m",
                str(area),
    ...
    if at_least("multistart"):
        ...
        _run(
            [PY, "scripts/run_sca_multistart_cases.py", "--skip-plot"],
            log_path,
        )
    ...
    if at_least("anchor"):
        ...
        _run(
            [
                PY,
                "scripts/run_sca_anchor_cases.py",
                "--only",
                only,
                "--skip-plot",
            ],
        ...
        _run([PY, "scripts/run_sca_anchor_lambda_aodt.py"], log_path)
        ...
        for n_runs in (100, 200):
            for area in (100, 500):
                _run(
                    [
                        PY,
                        "scripts/run_sca_anchor_highstat.py",
                        "--n-runs",
                        str(n_runs),
                        "--area-m",
                        str(area),
    ...
    if at_least("extras"):
        ...
        extras = [
            [PY, "scripts/check_bsys_20khz.py", "--n-runs", "20", "--out", "results/check_bsys_20khz.json"],
            [
                PY,
                "-m",
                "uavdt",
            ...
            [PY, "scripts/rerun_init_repair_points.py"],
            [PY, "scripts/paired_winrate.py", "results/campaign_8.8mhz_cap25_si12k.json"],
            [PY, "scripts/analyze_sca_vs_random_losses.py", "results/campaign_8.8mhz_cap25_si12k.json"],
            [PY, "scripts/analyze_campaigns.py"],
        ]
        for cmd in extras:
            try:
                _run(cmd, log_path)
    ...
    if at_least("plots"):
        ...
        plot_cmds = [
            ...
            [PY, "scripts/plot_sweep_n_stat_comparison.py"],
        ]
